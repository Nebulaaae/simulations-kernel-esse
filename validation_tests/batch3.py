...
# parser.add_argument("--addsimulation", action="store_true", help="Ajouter une simulation au dossier existant et additionner les projections aux volumes déjà présents (au lieu de les remplacer).")
...
for i in range(start_idx, NB_ANGLES):
    ...
    sum_p_2d = np.zeros((IMG_SIZE, IMG_SIZE))
    sum_s_2d = np.zeros((IMG_SIZE, IMG_SIZE))
    sum_t_2d_s3 = np.zeros((IMG_SIZE, IMG_SIZE))
    sum_t_2d_p = np.zeros((IMG_SIZE, IMG_SIZE))
    sum_t_2d_s4 = np.zeros((IMG_SIZE, IMG_SIZE))

    for j in range(SIM_PAR_ANGLE):
        print(f"    >>> Simulation Angle {angle:.1f}° n°{j+1}/{SIM_PAR_ANGLE}")
        subprocess.run([sys.executable, SIM_SCRIPT, str(angle), str(j * NB_ANGLES + i), "0", "8", str(BASE_ACTIVITY_MBq), str(PIXEL_SIZE), str(IMG_SIZE)], check=True)
        ...
        try:
            ...
            vol_p[i], vol_s[i], vol_t[i], vol_t_s3[i], vol_t_s4[i] = sum_p_2d, sum_s_2d, sum_t_2d_p, sum_t_2d_s3, sum_t_2d_s4
...
try:
    ...
    print(f"  ❌ Erreur génération atténuation : {e}")
